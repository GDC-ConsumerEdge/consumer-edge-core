...
function ingest_context() {
    ...
    if [[ -n "$cp_vip" && "$cp_vip" != "null" ]]; then yq e -i ".control_plane_vip = \"${cp_vip}\" | .control_plane_vip style=\"\"" "$yaml_out"; fi
    if [[ -n "$in_vip" && "$in_vip" != "null" ]]; then yq e -i ".ingress_vip = \"${in_vip}\" | .ingress_vip style=\"\"" "$yaml_out"; fi
    if [[ -n "$lb_pool" && "$lb_pool" != "null" ]]; then yq e -i ".load_balancer_pool_cidr = \"${lb_pool}\" | .load_balancer_pool_cidr style=\"\"" "$yaml_out"; fi
    ...
    if [[ -n "$storage" && "$storage" != "null" ]]; then yq e -i ".storage_provider = \"${storage}\" | .storage_provider style=\"\"" "$yaml_out"; fi
    if [[ -n "$abm_ver" && "$abm_ver" != "null" ]]; then yq e -i ".abm_version = \"${abm_ver}\" | .abm_version style=\"\"" "$yaml_out"; fi
    if [[ -n "$acm_ver" && "$acm_ver" != "null" ]]; then yq e -i ".acm_version = \"${acm_ver}\" | .acm_version style=\"\"" "$yaml_out"; fi
    ...
    if [[ "$storage" == "robin" ]]; then
        ...
        if [[ "$num_disks" -gt 0 && "$num_disks" != "null" ]]; then
            ...
            for (( i=0; i<$num_disks; i++ )); do
                ...
                yq e -i ".robin_disk_paths += [\"${disk_path}\"] | .robin_disk_paths[-1] style=\"\"" "$yaml_out"
            done
        fi
        ...
        if [[ -n "$robin_bundle" && "$robin_bundle" != "null" ]]; then
             yq e -i ".robin_install_bundle_file = \"${robin_bundle}\" | .robin_install_bundle_file style=\"\"" "$yaml_out"
        fi
    fi
    ...
    if [[ "$num_nodes" -gt 0 && "$num_nodes" != "null" ]]; then
        ...
        for n_name in $node_names; do
            ...
            yq e -i ".nodes += [{\"name\": \"${n_name}\", \"ip\": \"${n_ip}\"}] | .nodes[-1] style=\"\"" "$yaml_out"
        done
    fi
...
function generate_context() {
    ...
    yq e -i ".[\"${inv_cl_name}_cluster\"].vars.control_plane_vip = \"${cp_vip}\" | .[\"${inv_cl_name}_cluster\"].vars.control_plane_vip style=\"\"" "$target/inventory.yaml"
    yq e -i ".[\"${inv_cl_name}_cluster\"].vars.ingress_vip = \"${in_vip}\" | .[\"${inv_cl_name}_cluster\"].vars.ingress_vip style=\"\"" "$target/inventory.yaml"
    yq e -i ".[\"${inv_cl_name}_cluster\"].vars.load_balancer_pool_cidr = [\"${lb_pool}\"] | .[\"${inv_cl_name}_cluster\"].vars.load_balancer_pool_cidr[0] style=\"\"" "$target/inventory.yaml"
    ...
    for (( i=0; i<$num_nodes; i++ )); do
        ...
        yq e -i ".[\"${inv_cl_name}_cluster\"].hosts.\"${n_name}\".node_ip = \"${n_ip}\" | .[\"${inv_cl_name}_cluster\"].hosts.\"${n_name}\" style=\"\" | .[\"${inv_cl_name}_cluster\"].hosts.\"${n_name}\".node_ip style=\"\"" "$target/inventory.yaml"
        ...
        yq e -i ".[\"${inv_cl_name}_cluster\"].vars.peer_node_ips += [\"${n_ip}\"] | .[\"${inv_cl_name}_cluster\"].vars.peer_node_ips[-1] style=\"\"" "$target/inventory.yaml"
    ...
    if [[ -n "$storage_provider" && "$storage_provider" != "null" ]]; then
        yq e -i ".storage_provider = \"${storage_provider}\" | .storage_provider style=\"\"" "$target/instance-run-vars.yaml"
        ...
        if [[ "$storage_provider" == "robin" ]]; then
            ...
            if [[ "$num_disks" -gt 0 && "$num_disks" != "null" ]]; then
                ...
                for (( i=0; i<$num_disks; i++ )); do
                    ...
                    yq e -i ".robin_disk_paths += [\"${disk_path}\"] | .robin_disk_paths[-1] style=\"\"" "$target/instance-run-vars.yaml"
                done
            fi
        fi
    fi
    ...
    if [[ -n "$abm_version" && "$abm_version" != "null" ]]; then
        yq e -i ".abm_version = \"${abm_version}\" | .abm_version style=\"\"" "$target/instance-run-vars.yaml"
    fi

    if [[ -n "$acm_version" && "$acm_version" != "null" ]]; then
        yq e -i ".acm_version = \"${acm_version}\" | .acm_version style=\"\"" "$target/instance-run-vars.yaml"
    fi

    if [[ "$storage_provider" == "robin" && -n "$robin_bundle" && "$robin_bundle" != "null" ]]; then
        yq e -i ".robin_install_bundle_file = \"${robin_bundle}\" | .robin_install_bundle_file style=\"\"" "$target/instance-run-vars.yaml"
    fi
...
function create_context() {
    ...
    # 3. YAML Update
    if command -v yq &> /dev/null; then
        yq e -i ".context_name = \"${name}\"" "$config_yaml"
        if [[ -n "$project_id" && "$project_id" != "(unset)" && "$project_id" != "null" ]]; then
            yq e -i ".project_id = \"${project_id}\"" "$config_yaml"
        fi
    else
    ...
    if [[ "$answer" == "y" ]]; then
        ...
        if [[ "$p_id" == "null" || -z "$p_id" || "$p_id" == "your-gcp-project-id" ]]; then
             ...
             if [[ -n "$p_id" && "$p_id" != "null" && "$p_id" != "(unset)" && "$p_id" != "your-gcp-project-id" ]]; then
                 yq e -i ".project_id = \"${p_id}\"" "$config_yaml"
             fi
        fi
